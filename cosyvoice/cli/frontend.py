...
class CosyVoiceFrontEnd:
    ...
    def _extract_speech_token(self, prompt_wav):
        # ---------- cache hit ----------
        if prompt_wav in self._speech_token_cache:
            return self._speech_token_cache[prompt_wav]
        ...
        # ---------- cache store ----------
        self._speech_token_cache[prompt_wav] = (speech_token, speech_token_len)

        return speech_token, speech_token_len

    def _extract_spk_embedding(self, prompt_wav):
        # 固定 cache 目录
        emb_dir = "CosyVoice/spk_cache"
        os.makedirs(emb_dir, exist_ok=True)

        # 以 wav 文件名作为 cache key
        wav_name = os.path.splitext(os.path.basename(prompt_wav))[0]
        emb_path = os.path.join(emb_dir, f"{wav_name}.pt")

        # 1. cache 命中：直接加载
        if os.path.exists(emb_path):
            embedding = torch.load(emb_path, map_location=self.device)
            print("spk_emb cache hit")
        ...
        # 3. 保存 cache（统一保存为 CPU，避免跨设备问题）
        torch.save(embedding.cpu(), emb_path)
        print("cloneing spk_emb first appear, save cache in /data/yhy/CosyVoice/spk_cache")
    ...
    def frontend_zero_shot(self, tts_text, prompt_text, prompt_wav, resample_rate, zero_shot_spk_id):
        """
        Frontend with detailed timeline logging.
        """
        ...
        if zero_shot_spk_id == '':
            ...
            logging.info(
                "[TIMELINE][frontend_zero_shot] "
                "text_token=%.3fs | "
                "prompt_text_token=%.3fs | "
                "speech_feat=%.3fs | "
                "speech_token=%.3fs | "
                "spk_embedding=%.3fs | "
                "assemble=%.3fs | "
            ...
            logging.info(
                "[TIMELINE][frontend_zero_shot][cache] total=%.3fs",
                t1 - t0,
            )

        # ---------- append synthesis text ----------
